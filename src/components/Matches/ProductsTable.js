...
const ProductsTable = ({ matches = [], loading, onMatchDeleted, onMatchEdit }) => {
	...
	useEffect(() => {
		// Always ensure filteredMatches is an array, even if matches is undefined/null
		setFilteredMatches(Array.isArray(matches) ? matches : []);
	}, [matches]);

	const handleSearch = (e) => {
		const term = e.target.value.toLowerCase();
		setSearchTerm(term);
		const filtered = (Array.isArray(matches) ? matches : []).filter(
			(match) =>
